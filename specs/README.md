...
| Story | Título | Estado |
|---|---|---|
| [STORY-1020](STORY-1020.md) | Volver contextual: el "← Volver" del detalle de gestión pasa de `/tablero` fijo a `router.back()` con fallback al tablero (o `/tecnico`) — vuelve a Finanzas/cartera/técnico/auditoría/archivo, donde sea que se abrió; la pestaña de Finanzas vive en la URL (`?tab=`) para que el back restaure Cobros/Liquidaciones/Adelantos exacta. Los breadcrumbs "← Técnicos"/"← Administración" no se tocan (nombran su destino) | 🔨 implementada, sin commitear |
