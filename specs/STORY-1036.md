...
- **Implementación (2026-07-22):** 9 archivos, cero tablas/migraciones.
  ...
  - **v1.1 (fix visual, commit `bfb949d`):** el campo "Paga" del detalle truncaba (`Dato` con `truncate`) y cortaba el reparto compartido a la mitad ("Compartido (inquilino 20% / p…"). `Dato` gana prop `wrap` (usa `text-balance` en vez de `truncate`); la card "Paga" la activa solo con `pagador === "compartido"`. Dirección/email siguen truncando.
