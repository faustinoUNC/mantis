...
- **Verificación:** simulación SQL del rol gestor_administrativo post-fix: `recargo_tarjeta_*`, `medio_cobro_2`/`cobrado_monto_2` y `adelanto_materiales` pasan; `etapa` sigue bloqueada (`etapa_solo_por_funcion`) y una columna no financiera (`descripcion`) sigue dando `sin_permiso`. E2E en la app como admin: cobro tarjeta simple y combinado avanzan a Liquidación técnico. **E2E como gestor financiero** (Laura Benítez, usuaria demo): combinado tarjeta Medio 1 + efectivo + recargo 10% → registra y avanza a Liquidación técnico (evento con total $833.000 correcto). Nota: la password de Laura se restauró al valor documentado del seed (`usuario123`) porque el login fallaba.
