...
2. **Ubicación**: sacar la card del embudo de la caja "En el período" y ponerla en un bloque propio de estado actual (`Bloque titulo="Embudo en tiempo real"`, `cols={1}`), junto al resto de las vistas "ahora" (arriba de la caja del período). Badge `alcance="ahora"` (el tooltip ya dice "Estado actual — no cambia con el período").
