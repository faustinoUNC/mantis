...
- **v1.3 (misma noche):** los filtros Usuario y Tipo de evento pasan de select nativo a **combo filtrable** (pedido de Fausti: "que se pueda escribir y se vaya filtrando el desplegable"). Componente nuevo reutilizable `components/ui/combo-filtrable.client.tsx` (`ComboFiltrable`): input + listbox absoluto (patrón calcado de `BuscadorDireccion`), búsqueda sin tildes/mayúsculas, teclado (↑/↓/Enter/Escape), opción "Todos" fija arriba, grupos con encabezado — la clasificación visual de la v1.1 se conserva. Tipos de evento también clasificados: Gestiones en 6 grupos (Gestión / Asignación / Presupuesto / Ejecución / Conformidad / Cobro y liquidación), Sistema en 2 (Empleados / Técnicos); guardrail: un tipo sin clasificar cae al grupo "Otros", nunca desaparece del filtro. Verificado en navegador: tipeo filtra en vivo manteniendo grupos, selección dispara la query (counts correctos), ambos tabs.
