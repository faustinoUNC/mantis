...
- **Reaparición NEUTRA (revisado el 2026-07-24, pedido de Fausti):** la primera versión afirmaba *"el arreglo de fondo no aguantó"* cuando reaparecía tras una fondo terminada. Se **descartó** por overclaim: la detección agrupa por **rubro**, no por causa — una obra nueva de cerrajería podría ser otra cosa (copia de llaves, otra cerradura), no el fracaso del arreglo del picaporte. Afirmar la causa es el mismo pecado que cuidamos en Walter (sentenciar en vez de proponer). Queda un badge **NEUTRO "Volvió"** ("esto ya lo miraste y hay algo nuevo, revisá"); quién juzga si es la misma causa es el humano, o Walter leyendo las notas.
...
9. **Reaparición neutra:** una fila atendida (descartada o con gestión de fondo iniciada) que recibe una obra nueva del rubro vuelve con un badge **neutro "Volvió"** — NO se afirma la causa (la detección agrupa por rubro, no por causa; juzga el humano o Walter). Si la gestión de fondo se **cancela**, la fila reaparece de inmediato.
...
- **Verificación:** `tsc --noEmit` + eslint + `next build` verdes. **E2E navegador OK** (2026-07-24, admin, dev :3000, data real, Playwright):
  ...
  - **Reaparecer:** insertada una obra nueva de plomería en General Paz posterior al descarte → reaparece con badge **"Volvió"** (neutro), cuenta 4→5. (El motivo causal que se probó en su momento se removió después por overclaim — ver Decisión de diseño.)
