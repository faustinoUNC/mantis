# STORY-987 — Asignar técnico: lista compacta en vez de cards con 7 métricas (v1.2)
...
La lista de técnicos disponibles se ordena por **estrellas de mayor a menor** (los mejor calificados primero; los sin calificación quedan al final, desempate por nombre para orden estable). El orden se hace en `tecnicosDisponibles` (`features/gestiones/service.ts`), así el primero queda además pre-seleccionado. — commit `ef099fb`.
