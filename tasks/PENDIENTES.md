...
- [x] **Validaciones de flujo + archivo de finalizadas** (2026-07-11, STORY-935, SIN commitear — Fausti revisa) — (1) no se crea gestión si no hay técnico activo con la especialidad (tablero e inbox); (2) el email del presupuesto al pagador es obligatorio antes de "Aprobar y ejecutar" (marca `presupuesto_enviado_en`, bloqueo UI + server); (3) el resumen de obras ahora saluda "Hola, {propietario}:" (era el único email sin nombre); (4) archivar finalizadas + vista "Archivo" (`/gestiones/archivadas`, nav de staff, RLS respeta ownership del gestor, desarchivable). Verificada E2E.
